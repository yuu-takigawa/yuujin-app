...
export async function register(email: string, password: string, code: string): Promise<AuthResponse> {
  const result = await post<ServerAuthResult>('/auth/register', { email, password, code });
  setTokens(result.token, result.refreshToken);
